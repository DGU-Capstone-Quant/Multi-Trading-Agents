...
from modules.debate.paths import LOG_DIR
...
class BullNode(BaseNode):
    ...
    def run(self, **kwargs):
        ...
        try:
            ...
            _write_json(fp, {"time": datetime.now().isoformat(), "kwargs": {k: kwargs[k] for k in ["history","current_response","count"] if k in kwargs}})
        ...
        try:
            ...
            if report_dir:
                ...
                if report_path:
                    ...
                    from modules.debate.paths import RESULTS_DIR
